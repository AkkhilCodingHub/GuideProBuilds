...
async createSavedBuild(build: Partial<ISavedBuild>): Promise<ISavedBuild> {
  ...
  private async ensureSeeded(): Promise<void> {
    try {
      const count = await Part.countDocuments().exec();
      if (count === 0) {
        console.log('[Storage] Database is empty. Triggering live PCPartPicker fetch via Python API...');
        const { pcppService } = await import('./pcpp-service');
        await pcppService.syncPCPPToDatabase();
      }
    } catch (e) {
      console.error('[Storage] Error during live PCPartPicker fetch:', e);
    }
  }
