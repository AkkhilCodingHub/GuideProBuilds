...
const BILLING_EMAIL = process.env.SUPPORT_EMAIL || process.env.GMAIL_USER || 'akkhilsharmaclass@gmail.com';
const SUPPORT_EMAIL = BILLING_EMAIL;
...
async function sendEmailViaProvider(options: {
  ...
  const fromEmail = process.env.GMAIL_USER || 'akkhilsharmaclass@gmail.com';
  ...
  if (brevoApiKey) {
    try {
      const response = await fetch('https://api.brevo.com/v3/smtp/email', {
        ...
        body: JSON.stringify({
          sender: { name: 'PC Guide Pro', email: fromEmail },
          to: [{ email: options.to }],
  ...
  if (resendApiKey) {
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        console.log(`[Resend API] Email sent successfully. Message ID: ${data.id}`);
  ...
  console.log('\n======================================================');
  console.log('📬 [EMAIL DISPATCH - DEV CONSOLE MODE]');
  console.log(`FROM: ${fromEmail}`);
  console.log(`TO: ${options.to}`);
  console.log(`SUBJECT: ${options.subject}`);
  console.log(`REPLY-TO: ${options.replyTo || 'N/A'}`);
  console.log('------------------------------------------------------');
  console.log(`(Email logged to console because BREVO_API_KEY is not set)`);
  console.log('======================================================\n');
